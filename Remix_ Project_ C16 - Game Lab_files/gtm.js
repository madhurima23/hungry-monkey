...
var em=function(a,b,c){function d(){var h=a();f+=e?(La()-e)*h.playbackRate/1E3:0;e=La()}var e=0,f=0;return{fc:function(h,k,l){var n=a(),q=n.Pe,r=void 0!==l?Math.round(l):void 0!==k?Math.round(n.Pe*k):Math.round(n.Og),t=void 0!==k?Math.round(100*k):0>=q?0:Math.round(r/q*100),p=H.hidden?!1:.5<=mj(c);d();var u=Il(c,"gtm.video",[b]);u["gtm.videoProvider"]="youtube";u["gtm.videoStatus"]=h;u["gtm.videoUrl"]=n.url;u["gtm.videoTitle"]=n.title;u["gtm.videoDuration"]=Math.round(q);u["gtm.videoCurrentTime"]=
Math.round(r);u["gtm.videoElapsedTime"]=Math.round(f);u["gtm.videoPercent"]=t;u["gtm.videoVisible"]=p;yl(u)},Nh:function(){e=La()},Xc:function(){d()}}};var fm=F.clearTimeout,gm=F.setTimeout,N=function(a,b,c){if(vg()){b&&I(b)}else return Xc(a,b,c)},hm=function(){return new Date},im=function(){return F.location.href},jm=function(a){return Ge(Ie(a),"fragment")},km=function(a){return He(Ie(a))},lm=function(a,b){return re(a,b||2)},mm=function(a,b,c){var d;b?(a.eventCallback=b,c&&(a.eventTimeout=c),d=yl(a)):d=yl(a);return d},nm=function(a,b){F[a]=b},X=function(a,b,c){b&&
...
Z.a.ytl=["google"],function(){function a(){var v=Math.round(1E9*Math.random())+"";return H.getElementById(v)?a():v}function b(v,x){if(!v)return!1;for(var y=0;y<r.length;y++)if(0<=v.indexOf("//"+r[y]+"/"+x))return!0;return!1}function c(v){var x=-1!==v.indexOf("?")?"&":"?";if(-1<v.indexOf("origin="))return v+x+"enablejsapi=1";if(!p){var y=X("document");p=y.location.protocol+"//"+y.location.hostname;y.location.port&&(p+=":"+y.location.port)}return v+x+"enablejsapi=1&origin="+encodeURIComponent(p)}function d(v,
x){var y=v.getAttribute("src");if(b(y,"embed/")){if(0<y.indexOf("enablejsapi=1"))return!0;if(x)return v.setAttribute("src",c(y)),!0}return!1}function e(v,x){if(!v.getAttribute("data-gtm-yt-inspected-"+x.Fd)&&(v.setAttribute("data-gtm-yt-inspected-"+x.Fd,"true"),d(v,x.Te))){v.id||(v.id=a());var y=X("YT"),w=y.get(v.id);w||(w=new y.Player(v.id));var B=h(w,x),z={},A;for(A in B)z.Ya=A,B.hasOwnProperty(z.Ya)&&w.addEventListener(z.Ya,function(E){return function(G){return B[E.Ya](G.data)}}(z)),z={Ya:z.Ya}}}
function f(v){I(function(){function x(){for(var w=y.getElementsByTagName("iframe"),B=w.length,z=0;z<B;z++)e(w[z],v)}var y=X("document");x();Ul(x)})}function h(v,x){var y,w;function B(){ca=em(function(){return{url:P,title:S,Pe:K,Og:v.getCurrentTime(),playbackRate:T}},x.Fd,v.getIframe());K=0;S=P="";T=1;return z}function z(U){switch(U){case t.PLAYING:K=Math.round(v.getDuration());P=v.getVideoUrl();if(v.getVideoData){var ja=v.getVideoData();S=ja?ja.title:""}T=v.getPlaybackRate();x.Ig?ca.fc("start"):ca.Xc();
J=n(x.Fh,x.Eh,v.getDuration());return A(U);default:return z}}function A(){na=v.getCurrentTime();R=hm().getTime();ca.Nh();W();return E}function E(U){var ja;switch(U){case t.ENDED:return M(U);case t.PAUSED:ja="pause";case t.BUFFERING:var Ja=v.getCurrentTime()-na;ja=1<Math.abs((hm().getTime()-R)/1E3*T-Ja)?"seek":ja||"buffering";v.getCurrentTime()&&(x.Hg?ca.fc(ja):ca.Xc());V();return G;case t.UNSTARTED:return B(U);default:return E}}function G(U){switch(U){case t.ENDED:return M(U);case t.PLAYING:return A(U);
case t.UNSTARTED:return B(U);default:return G}}function M(){for(;w;){var U=y;fm(w);U()}x.Gg&&ca.fc("complete",1);return B(t.UNSTARTED)}function O(){}function V(){w&&(fm(w),w=0,y=O)}function W(){if(J.length&&0!==T){var U=-1,ja;do{ja=J[0];if(ja.Ia>v.getDuration())return;U=(ja.Ia-v.getCurrentTime())/T;if(0>U&&(J.shift(),0===J.length))return}while(0>U);y=function(){w=0;y=O;0<J.length&&J[0].Ia===ja.Ia&&(J.shift(),ca.fc("progress",ja.$e,ja.df));W()};w=gm(y,1E3*U)}}var ca,J=[],K,P,S,T,na,R,fa=B(t.UNSTARTED);
w=0;y=O;return{onStateChange:function(U){fa=fa(U)},onPlaybackRateChange:function(U){na=v.getCurrentTime();R=hm().getTime();ca.Xc();T=U;V();W()}}}function k(v){for(var x=v.split(","),y=x.length,w=[],B=0;B<y;B++){var z=parseInt(x[B],10);isNaN(z)||100<z||0>z||w.push(z/100)}w.sort(function(A,E){return A-E});return w}function l(v){for(var x=v.split(","),y=x.length,w=[],B=0;B<y;B++){var z=parseInt(x[B],10);isNaN(z)||0>z||w.push(z)}w.sort(function(A,E){return A-E});return w}function n(v,x,y){var w=v.map(function(A){return{Ia:A,
df:A,$e:void 0}});if(!x.length)return w;var B=x.map(function(A){return{Ia:A*y,df:void 0,$e:A}});if(!w.length)return B;var z=w.concat(B);z.sort(function(A,E){return A.Ia-E.Ia});return z}function q(v){var x=!!v.vtp_captureStart,y=!!v.vtp_captureComplete,w=!!v.vtp_capturePause,B=k(v.vtp_progressThresholdsPercent+""),z=l(v.vtp_progressThresholdsTimeInSeconds+""),A=!!v.vtp_fixMissingApi;if(x||y||w||B.length||z.length){var E={Ig:x,Gg:y,Hg:w,Eh:B,Fh:z,Te:A,Fd:void 0===v.vtp_uniqueTriggerId?"":v.vtp_uniqueTriggerId},
G=X("YT"),M=function(){f(E)};I(v.vtp_gtmOnSuccess);if(G)G.ready&&G.ready(M);else{var O=X("onYouTubeIframeAPIReady");nm("onYouTubeIframeAPIReady",function(){O&&O();M()});I(function(){for(var V=X("document"),W=V.getElementsByTagName("script"),ca=W.length,J=0;J<ca;J++){var K=W[J].getAttribute("src");if(b(K,"iframe_api")||b(K,"player_api"))return}for(var P=V.getElementsByTagName("iframe"),S=P.length,T=0;T<S;T++)if(!u&&d(P[T],E.Te)){N("https://www.youtube.com/iframe_api");u=!0;break}})}}else I(v.vtp_gtmOnSuccess)}
var r=["www.youtube.com","www.youtube-nocookie.com"],t={UNSTARTED:-1,ENDED:0,PLAYING:1,PAUSED:2,BUFFERING:3,CUED:5},p,u=!1;(function(v){Z.__ytl=v;Z.__ytl.b="ytl";Z.__ytl.g=!0;Z.__ytl.priorityOverride=0})(function(v){v.vtp_triggerStartOption?q(v):dh(function(){q(v)})})}();
